<#assign active="page" title="保存页面" />
<#include "header.ftl" />
<link href="${request.contextPath}/static${request.contextPath}/admin/plugins/toggles/toggles.css" rel="stylesheet">
            ...
            <div class="form-group col-md-3 col-sm-4">
                ...
                <div class="col-sm-8">
                    <div class="toggle toggle-success allow-${((content.allowComment)!true)?c}"
                         onclick="allowComment(this);" on="${((content.allowComment)!true)?c}"></div>
                </div>
            </div>
...
<script type="text/javascript">
        ...
        htmlEditor = $('.summernote').summernote({
            ...
            callbacks:{
                onImageUpload: function(files) {
                    ...
                    tale.showLoading();
                    $.ajax({
                        url: '${request.contextPath}${request.contextPath}/admin/attach/upload',     //上传图片请求的路径
                        method: 'POST',            //方法
                        data:data,                 //数据
    ...
    function  autoSave() {
        ...
        if (title != '' && content != '') {
            ...
            var url = cid != '' ? '${request.contextPath}/admin/page/modify' : '${request.contextPath}/admin/page/publish';
            tale.post({
                url: url,
                data: params,
                success: function (result) {
    ...
    function subArticle(status) {
        ...
        var params = $("#articleForm").serialize();
        var url = $('#articleForm #cid').val() != '' ? '${request.contextPath}/admin/page/modify' : '${request.contextPath}/admin/page/publish';
        tale.post({
            url: url,
            data: params,
            success: function (result) {
                if (result && result.success) {
                    tale.alertOk({
                        ...
                        then: function () {
                            setTimeout(function () {
                                window.location.href = '${request.contextPath}/admin/page';
                            }, 500);
                        }
                    });
                } else {
